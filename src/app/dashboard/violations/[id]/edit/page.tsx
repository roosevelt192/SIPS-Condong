...
export default function EditViolationPage({
  ...
  useEffect(() => {
    async function loadViolationAndStudent() {
      try {
        setLoading(true);

        // 1. Fetch Violation Record
        ...
        if (!vData) throw new Error("Catatan pelanggaran tidak ditemukan.");

        setViolationName(vData.violation_name || "");
        setCategory(vData.category || "Ringan");
        setPoints(Number(vData.points) || 5);
        setSanction(vData.sanction || "");
        setStatus(vData.status || "Proses");
        setDescription(vData.description || "");
        setExistingDocUrl(vData.document_url || null);
        ...
        if (vData.student_id) {
          const { data: sData } = await supabase
            .from("students")
            .select("*")
            .eq("id", vData.student_id)
            .single();

          if (sData) {
            setStudent(mapStudentData(sData));
          }
        } else if (vData.nis) {
          const { data: sData } = await supabase
            .from("students")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!violationName.trim()) {
      setErrorMessage("Bentuk pelanggaran wajib diisi.");
      return;
    }
    ...
    try {
      ...
      if (newFile && student) {
        ...
        const filePath = `documents/${student.nis}_${Date.now()}.${fileExt}`;
        ...
        if (!uploadError) {
          ...
          documentUrl = publicUrlData?.publicUrl || documentUrl;
        }
      }
      ...
      setErrorMessage("Gagal memperbarui catatan pelanggaran: " + err.message);
      setSubmitting(false);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-3">
        ...
        <p className="text-xs font-bold text-slate-500">Memuat berkas kedisiplinan santri...</p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-20 font-sans relative">
      ...
      <div className="relative overflow-hidden rounded-[32px] border border-slate-200/80 dark:border-slate-800/80 bg-white/90 dark:bg-slate-900/90 p-6 sm:p-8 shadow-2xl backdrop-blur-2xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5">
        <div className="flex items-center space-x-4">
          <Link
            ...
            className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 hover:text-rose-500 hover:border-rose-500/40 hover:bg-rose-500/10 transition-all duration-200 active:scale-90 shadow-sm"
            title="Kembali ke Daftar Pelanggaran"
          >
            <ArrowLeft className="h-5 w-5 stroke-[2.5]" />
          </Link>

          <div className="relative flex h-13 w-13 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-tr from-cyan-500/20 via-blue-500/20 to-indigo-500/20 text-cyan-500 border border-cyan-500/30 shadow-inner">
            <Edit3 className="h-6 w-6 stroke-[2.3]" />
          </div>

          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">
                Edit Catatan Disiplin & Sanksi
              </h1>
              <span className="inline-flex items-center gap-1.5 rounded-full bg-cyan-500/10 px-3 py-1 text-[11px] font-black uppercase text-cyan-600 dark:text-cyan-400 border border-cyan-500/20 shadow-sm">
                <Sparkles className="h-3.5 w-3.5" /> Modifikasi Data
              </span>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">
              Perbarui rincian pelanggaran, bobot poin, status pembinaan, serta berkas surat pernyataan
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={submitting}
            className="relative group inline-flex items-center space-x-2.5 rounded-2xl bg-gradient-to-r from-rose-600 via-rose-500 to-amber-500 px-7 py-3.5 text-xs sm:text-sm font-black text-white shadow-xl shadow-rose-500/25 hover:shadow-rose-500/40 hover:scale-[1.03] active:scale-95 transition-all duration-300 disabled:opacity-50 overflow-hidden cursor-pointer"
          >
            <Send className="h-4 w-4 stroke-[2.5]" />
            <span>{submitting ? (uploadingFile ? "Mengunggah..." : "Menyimpan...") : "Simpan Perubahan"}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          {student ? (
            <div className="relative overflow-hidden rounded-[28px] border-2 border-cyan-500/40 bg-white dark:bg-slate-900 shadow-2xl transition-all duration-300">
              {/* Header Pita Kartu */}
              ...
              <div className="p-6 bg-slate-50/50 dark:bg-slate-900/60 relative">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 relative z-10">
                  {/* Foto Besar Santri */}
                  <div className="relative h-[180px] w-[135px] shrink-0 rounded-2xl overflow-hidden border-[5px] border-white dark:border-slate-800 bg-gradient-to-tr from-cyan-600 to-blue-500 shadow-xl">
                    {student.photo_url ? (
                      <Image
                  ...
                  <div className="flex-1 w-full space-y-3 text-xs text-slate-800 dark:text-slate-200">
                    ...
                    <div className="grid grid-cols-12 items-start">
                      ...
                      <span className="col-span-7 font-mono font-medium text-slate-600 dark:text-slate-400">
                        ...
                        {student.guardian_name && student.guardian_name !== "-" ? ` (${student.guardian_name})` : ""}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        {/* KOLOM KANAN: FORM EDIT PELANGGARAN */}
          ...
          <div className="rounded-[32px] border border-slate-200/90 dark:border-slate-800/90 bg-white/90 dark:bg-slate-900/90 p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
            ...
            <div className="space-y-2.5">
              <label className="block text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center justify-between">
                ...
                <span className="text-[10px] text-cyan-600 dark:text-cyan-400 font-bold">Klik opsi untuk mengganti cepat</span>
              </label>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="space-y-3 pt-2">
                <label className="block text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                  ...
                  Berkas / Surat Pernyataan Terlampir
                </label>
                ...
                <div
                  ...
                  className="cursor-pointer border-2 border-dashed border-slate-200 dark:border-slate-800 hover:border-rose-500/60 rounded-2xl p-4 text-center bg-slate-50/50 dark:bg-slate-950/50 hover:bg-rose-50/20 transition-all"
                >
                  <input
                    ref={fileInputRef}
                    type="file"
                  ...
                  <p className="text-xs font-bold text-slate-700 dark:text-slate-300 mt-1">
                    {newFile ? `File baru terpilih: ${newFile.name}` : "Klik untuk mengunggah dokumen baru / pengganti"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
              ...
              <button
                ...
                onClick={handleSubmit}
                disabled={submitting}
                className="group relative inline-flex items-center space-x-2.5 rounded-2xl bg-gradient-to-r from-rose-600 via-rose-500 to-amber-500 px-8 py-3.5 text-xs sm:text-sm font-black text-white shadow-xl shadow-rose-500/25 hover:shadow-rose-500/40 hover:scale-[1.03] active:scale-95 transition-all duration-300 disabled:opacity-50 overflow-hidden cursor-pointer"
              >
                <Send className="h-4 w-4 stroke-[2.5]" />
                <span>{submitting ? (uploadingFile ? "Mengunggah..." : "Menyimpan...") : "Simpan Perubahan"}</span>
              </button>
